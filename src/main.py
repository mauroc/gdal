...
def parse_chart(file_name,chart_name):
	global chart_count, driver, tot_objs, s57classes
	print file_name
	chart_count+=1
...
def sql_create_obj(chart_name, s57class, feat):
	global db, cur, s57classes, tot_unique_objs
	...
	if sql_find_obj(lnam, s57class)!=():		
		print("feature already in database: %s %s" %(lnam, s57class))
	else:
		tot_unique_objs += 1
		...
		add_obj = ("INSERT INTO s57objs ( f_class, lnam, latitude, longitude,chart_name, tags, created_at, updated_at) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)")
...
chart_count = tot_objs = tot_unique_objs = 0
